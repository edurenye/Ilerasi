...
class CheckoutController extends Zend_Controller_Action
{
    ...
    public function showAction()
    {
        ...
        $select_fac->from('factura', array(id_factura, total))
        ...
        $total = 0;
        $total = $total + $row_res->preu;
        foreach ($rows_spa as $valor_spa) {
            $total = $total + $valor_spa->preu;
        }
        foreach ($rows_con_men as $valor_res) {
            $total = $total + $valor_res->preu;
        }
        foreach ($rows_usa_pro as $valor_min) {
            $preu_min = $valor_min->preu * $valor_min->quantitat;
            $total = $total + $preu_min;
        }
        foreach ($rows_mas as $valor_mas) {
            $total = $total + $valor_mas->preu;
        }

        $preu_fac = $row_fac->total;

        if ($total == $preu_fac) {
            echo "<script type='text/javascript'>alert('El client ja ha fet el Check-out');</script>";
            exit;
        }
        else {
            $data_fac = array(
            ...
            $where_fac['id_factura = ?'] = $id_factura;
            $table_fac->update($data_fac, $where_fac);

            $view->setScriptPath('/var/www/intranet/application/views/scripts');
            $view->render('/checkout/show.phtml');
        }
    }
}
